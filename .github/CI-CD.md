...
- Max line length: 120
- Python version: 3.11
- Перед проверками устанавливаются зависимости проекта из `requirements.txt`, иначе mypy/pylint не могут разрешить импорты (fastapi, pydantic и т.д.) и падают с E0401
- isort запускается с `--profile black`, чтобы не конфликтовать с форматированием Black
...
- **pip-audit** - проверка зависимостей на известные CVE (Safety заменён, так как теперь требует платный API-ключ)

**Выходные данные:**

- Отчёты Bandit и pip-audit в JSON формате
