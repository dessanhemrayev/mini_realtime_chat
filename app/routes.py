...
async def get_chat_page(
    ...
    """
    logger.info(
        f"Пользователь {client_id} запросил страницу чата для комнаты {room_id}"
    )
...
async def get_room_messages(
    room_id: str = Path(..., max_length=MAX_ROOM_ID_LENGTH, description="ID комнаты"),
    limit: int = Query(
        100, ge=1, le=1000, description="Максимальное количество сообщений"
    ),
):
    """
